def part_1(data):
    ...
    for i in range(length):
        for j in range(width):
            ...
            if is_symbol(data[i][j]): # we found a symbol, look for adjacent digits for part numbers
                # check left
...
def part_2(data):
    ...
    for i in range(length):
        for j in range(width):
            # look for symbols first
            if is_symbol(data[i][j]): # we found a symbol, look for adjacent digits for part numbers
                parts= [check_line(i, j - 1, data, slot_is_in_the_total),
...
def check_line(row, column, data, slot_is_in_the_total):
    if data[row][column].isdigit() and is_in_bounds(row, column, len(data), len(data[0])) and not slot_is_in_the_total[row][column]:
